...
def rotatedRectWithMaxArea(w: float, h: float, angle: float) -> tuple[float, ...]:
    """Compute the largest axis-aligned rectangle inside a rotated one.

    Given a rectangle of size wxh that has been rotated by 'angle' (in
    radians), computes the width and height of the largest possible
    axis-aligned rectangle (maximal area) within the rotated rectangle.
    Adapted from https://stackoverflow.com/questions/16702966.
    """
    if w <= 0 or h <= 0:
        return 0, 0
...
def compute_quadrilateral_area_from_corners(
    corners: Tensor | np.ndarray,
) -> Tensor | np.ndarray:
    """Compute the area of a quadrilateral from its 4 corners (shoelace)."""
    area = 0.5 * (
        (
...
def grid_sample_nan(
    ...
    """Sample an image at coordinates with nan handling.

    Pytorch grid_sample with embedded coordinate normalization and grid nan
    handling (if a nan is present in xy, the output will be nan). Works both
    with input of shape Bxnx2 and Bxn0xn1x2. xy points that fall outside the
    image are treated as nan (those which are really close are interpolated
    using border padding mode).

    Args:
        xy: input coordinates (with the convention top-left pixel center at
            (0.5, 0.5))
            B,n,2 or B,n0,n1,2
    ...
    Returns:
        ...
        mask_img_nan: mask of the points that had a nan in the img. The points
            xy that were nan appear as false in the mask, just like points with
            a valid img value. This discriminates between invalid sampling
            positions and valid ones with a nan value in the image
            BxN or BxN0xN1.
    """
    ...
    # points xy that are not nan and have nan img. The sum is just to squash
    # the channel dimension
    mask_img_nan = torch.isnan(sampled.sum(1))  # BxN or BxN0xN1
    # set to nan the sampled values for points xy that were nan (grid_sample
    # considers those as (-1, -1))
...
def normalize_pixel_coordinates(xy: Tensor, shape: tuple[int, int]) -> Tensor:
    """Normalize pixel coordinates to the [-1, +1] range.

    With (-1,-1) the exact top-left corner of the image, the coordinates must
    be given so that the center of a pixel is at half coordinates (0.5, 0.5).
    xy is ordered as (x, y) and shape as (H, W).

    Args:
        xy: input coordinates in order (x, y) with the convention top-left
            pixel center at coordinates (0.5, 0.5)
            ...x2
    ...
    """
    xy_norm = xy.clone()
    # ! the shape indices are flipped because the coordinates are given as x,y
    # but shape is H,W
...
def filter_outside(xy: Tensor, shape: tuple[int, int], border: int = 0) -> Tensor:
    """Set to nan all points outside the rectangle defined by shape HxW.
    ...
    Returns:
        The input keypoints with 'nan' where one of the two coordinates was
        not contained inside the shape, xy_filtered (B)xnx2.
    """
    assert xy.shape[-1] == 2
...
def find_mutual_nearest_neighbors_from_keypoints_and_their_projections(
    ...
    """Find mutual nearest neighbors between keypoints and projections."""
    ...
    assert xy0_proj.ndim == 2 and xy1_proj.ndim == 2, (
        f"xy0_proj and xy1_proj must be 2D tensors, got {xy0_proj.ndim} and "
        f"{xy1_proj.ndim}"
    )
    ...
    if n1 > 0:
        xy0_closest_dist, _ = dist1.min(1)
    else:
        xy0_closest_dist, _ = (
    ...
    if n0 > 0:
        xy1_closest_dist, _ = dist0.min(0)
    else:
        xy1_closest_dist, _ = (
            torch.zeros((0,), device=device),
            torch.zeros((0,), dtype=torch.long, device=device),
    ...
    # find the keypoints that are mutual nearest neighbors (using only x,y
...
def find_distance_matrices_between_points_and_their_projections(
    ...
    """Compute distance matrices between keypoints and their projections."""
    assert xy0.ndim == 2 and xy1.ndim == 2, (
        f"xy0 and xy1 must be 2D tensors, got {xy0.ndim} and {xy1.ndim}"
    )
    assert xy0_proj.ndim == 2 and xy1_proj.ndim == 2, (
...
def mutual_nearest_neighbors_from_score_matrix(
    ...
    # each row is the score between a descriptor from img0 and all the others
    # in img1
    # each column is the score between a descriptor from img1 and all the
    # others in img0
...
def mutual_nearest_neighbors_from_dist_matrices(dist0: Tensor, dist1: Tensor) -> Tensor:
    """Return the MNN boolean matrix from two distance matrices.

    A True marks a position that is a minimum in both the columns of dist0 and
    the rows of dist1.

    Args:
        dist0: distance matrix computed in img0 between rows xy0 and columns
            xy1_proj
            Bxn0xn1
        dist1: distance matrix computed in img1 between rows xy0_proj and
            columns xy1
            Bxn0xn1
    ...
    return mnn_matrix[:, :-1, :-1]


@dataclass
class MatchingMatrix:
    """Classified matching matrices (proposed/correct/wrong/...)."""
    ...
    def __getitem__(self, b: int = 0) -> "MatchingMatrix":
...
def compute_correct_wrong_mismatched_inexistent_unsure_matches(
    ...
    Returns:
        A MatchingMatrix with correct matches, wrong matches, mismatched
        matches (a correct match existed but the point was wrongly matched),
        and inexistent matches (a match between two points with no GT match).
    """
...
def add_bins_to_matching_matrix(matching_matrix: Tensor) -> Tensor:
    """Add an unmatched bin row and column to a matching matrix.
...
def extract_maxima_from_map(
    ...
    """Extract keypoints (local maxima) from the provided score map.

    Uses the convention top-left pixel center (0.5, 0.5) and the provided
    parameters.

    Args:
        score_map: the input map where to look for maxima
            BxHxW or Bx1xHxW
        thr: the maxima threshold; everything below this value is not extracted.
        nms_radius: the Non-Maxima-Suppression radius.
        border: border width to remove from each side.
        max_kpts: the max number of kpts to extract (top scores selected).
        edge_thr: avoid extracting points from the edges.

    Returns:
        kpts: the extracted keypoints for every image in the batch ordered by
            score
            len(kpts) = B, kpts.shape = Nb x 2 (x, y) with convention top-left
            pixel center (0.5, 0.5)
        scores: the scores of the extracted keypoints
    ...
    # if there are nans in the border of the score map, the border of the
    # max_pool+kernel_size//2 will be nan. this makes sense as if the max was
    # really in the edge before the nan, we can't trust it anyway
    ...
    scores_list = []
    for b in range(B):
        # transpose the map before the nonzero so the coordinates are (x, y)
        kpts = torch.nonzero(maxima_mask[b].permute(1, 0))  # Nx2  (x,y)
        kpts_scores = score_map[b][kpts[:, 1], kpts[:, 0]]  # N

        # if there are too many keypoints, keep only the ones with higher
        # score; in any case sort them by score
...
def generate_round_kernel_indices(kernel_radius: int) -> Tensor:
    """Generate the indices for a round mask centered at 0.
    ...
    return torch.tensor(mask_idxs, dtype=torch.long).view(
        -1, 2
    )  # mask_area x 2     (y, x)
