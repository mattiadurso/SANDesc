"""Keypoint manipulation and sampling utilities."""
...
def get_valid_keypoint_mask(
    ...
    """Return a mask of valid keypoints, i.e. those inside the patch.
...
def filter_outside_window(
    xy: Tensor, top_left_and_bottom_right: Tensor, border: int = 0
) -> Tensor:
    """Set to nan all points outside the rectangle.

    The rectangle is defined by ``top_left_and_bottom_right``.

    Args:
        xy: keypoints with coordinate (x, y)
            (B),n,2
        top_left_and_bottom_right: top-left and bottom-right coordinates of the
            rectangle (x, y)
            (B),2,2
    ...
    Returns:
        ...
        not contained inside the shape, xy_filtered (B),n,2.
    """
    assert xy.shape[-1] == 2, (
        f"the last dimension of xy must be 2, but is {xy.shape[-1]}"
    )
    assert top_left_and_bottom_right.shape[-2:] == (
        2,
        2,
    ), (
        "the last two dims of top_left_and_bottom_right must be 2,2 but are "
        f"{top_left_and_bottom_right.shape[-2:]}"
    )
...
def find_mutual_nearest_neighbors_from_keypoints_and_their_projections(
    ...
    Args:
        ...
        xy1_proj: second set of projected keypoints
            n1_proj,2
        dist0: optional distance matrix between xy0 and xy1_proj; computed if
            not provided
            n0,n1
        dist1: optional distance matrix between xy0_proj and xy1; computed if
            not provided
            n0,n1
    ...
    Returns:
        mnn_mask: binary mask of mutual nearest neighbors
            n0,n1
        xy0_closest_dist_mnn: for each xy0 with a mutual nearest neighbor, the
            distance to the closest xy1_proj in img0
            n0_mnn
        xy1_closest_dist_mnn: for each xy1 with a mutual nearest neighbor, the
            distance to the closest xy0_proj in img1
            n1_mnn
        xy0_closest_dist: for each xy0, the distance to the closest xy1_proj in
            img0
            n0
        xy1_closest_dist: for each xy1, the distance to the closest xy0_proj in
            img1
            n1.
    """
    ...
    if dist0 is not None:
        assert dist0.shape == (
            ...
            f"dist0 must be a matrix of shape ({xy0.shape[0]}, "
            f"{xy1.shape[0]}), got {dist0.shape}"
        )
    if dist1 is not None:
        assert dist1.shape == (
            ...
            f"dist1 must be a matrix of shape ({xy0.shape[0]}, "
            f"{xy1.shape[0]}), got {dist1.shape}"
        )
...
def find_distance_matrices_between_points_and_their_projections(
    ...
    # dist0 = torch.cdist(xy0.to(torch.float), xy1_proj,
    #     compute_mode='donot_use_mm_for_euclid_dist')  # n0,n1
    # dist1 = torch.cdist(xy0_proj, xy1.to(torch.float),
    #     compute_mode='donot_use_mm_for_euclid_dist')  # n0,n1
...
def nms_keypoints(
    kpts: Tensor, kpts_score: Tensor, nms_radius: float
) -> tuple[Tensor, Tensor]:
    """Suppress keypoints near a stronger neighbor within ``nms_radius``."""
    ...
    assert kpts.shape[0] == kpts_score.shape[0], (
        f"kpts and kpts_score must have the same number of keypoints, got "
        f"{kpts.shape[0]} and {kpts_score.shape[0]}"
    )

    dist = torch.cdist(kpts[None], kpts[None])[0]  # n,n
    # dist = torch.cdist(kpts[None], kpts[None],
    #     compute_mode='donot_use_mm_for_euclid_dist')[0]  # n,n
    dist[torch.eye(kpts.shape[0], dtype=torch.bool, device=kpts.device)] = float("inf")

    closest_idx = dist.argmin(-1)  # n
    closest_dist = dist[torch.arange(kpts.shape[0]), closest_idx]  # n
    closest_score = kpts_score[closest_idx]  # n
    suppression_mask: Tensor = (closest_dist <= nms_radius) * (
        closest_score > kpts_score
    )  # n
