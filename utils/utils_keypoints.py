...
def nms_keypoints(
    ...
    assert (
        kpts.shape[0] == kpts_score.shape[0]
    ), f"kpts and kpts_score must have the same number of keypoints, got {kpts.shape[0]} and {kpts_score.shape[0]}"

    dist = torch.cdist(kpts[None], kpts[None])[0]  # n,n
    # dist = torch.cdist(kpts[None], kpts[None], compute_mode='donot_use_mm_for_euclid_dist')[0]  # n,n
