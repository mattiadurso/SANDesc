"""Mutual nearest neighbor matcher used to build GT match matrices."""
...
@dataclass
class Matches:
    """Container for proposed matches and their score matrix."""
    ...
    def compute_scores_stats(
        ...
        #  find out how many possible mismatched have been shielded by a
        #  correct match. we do this by counting how many columns have the max
        #  score corresponding to a column where there is a correct match
        matches_correct_idx = (
            self.matching_matrix_extra.correct.nonzero()
        )  # n_matches_correct,2
        #  we first create a mask with a one in the position where the score
        #  is the max for that row
        row_max_mask = (
        ...
        #  and sum over those columns (subtracting always one as we do not
        #  want to count the correct match)
